...
class VNet(nn.Module):
    ...
    def __init__(
        ...
        self.in_tr = InputTransition(spatial_dims, in_channels, 15, act, bias=bias)
        self.down_tr32 = DownTransition(spatial_dims, 15, 1, act, bias=bias)
        self.down_tr64 = DownTransition(spatial_dims, 30, 2, act, bias=bias)
        self.down_tr128 = DownTransition(spatial_dims, 60, 3, act, dropout_prob=dropout_prob, bias=bias)
        self.down_tr256 = DownTransition(spatial_dims, 120, 2, act, dropout_prob=dropout_prob, bias=bias)
        self.up_tr256 = UpTransition(spatial_dims, 240, 240, 2, act, dropout_prob=dropout_prob)
        self.up_tr128 = UpTransition(spatial_dims, 240, 120, 2, act, dropout_prob=dropout_prob)
        self.up_tr64 = UpTransition(spatial_dims, 120, 60, 1, act)
        self.up_tr32 = UpTransition(spatial_dims, 60, 30, 1, act)
        self.out_tr = OutputTransition(spatial_dims, 30, out_channels, act, bias=bias)
...
if __name__ == "__main1__":
    ...
    out_ch = 4
    spatial_dim = 3
    ex_img = np.random.random((3, 96, 96, 144))
    ex_img = ex_img[np.newaxis, :, :, :, :]
    ex_img = torch.Tensor(ex_img)
    model = VNet(spatial_dim, in_ch, out_ch)
    ex_pred = model(ex_img)
    print("")
